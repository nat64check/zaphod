    ...
    with open(country_code_filename, newline='') as csv_file:
        for country_line in csv.DictReader(csv_file):
            ...
            if region and country not in region.countries:
                region.countries = sorted(region.countries + [country], key=lambda c: c.code)
                region.save()

            if sub_region and country not in sub_region.countries:
                sub_region.countries = sorted(sub_region.countries + [country], key=lambda c: c.code)
                sub_region.save()

            if int_region and country not in int_region.countries:
                int_region.countries = sorted(int_region.countries + [country], key=lambda c: c.code)
                int_region.save()
